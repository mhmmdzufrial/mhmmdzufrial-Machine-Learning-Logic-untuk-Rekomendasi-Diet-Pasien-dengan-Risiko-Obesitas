...
def preprocess_new_data(new_patient_data, df_existing, scaler_obj, fitur_final_training_data): # Membuat fungsi preprocessing data baru untuk metode CBF.
    ...
    for col in required_original_cols:                                                     # Melakukan perulangan untuk setiap kolom wajib.
        if col not in temp_df_single.columns:                                              # Mengecek apakah kolom belum tersedia.
            temp_df_single[col] = '' if temp_df_single[col].dtype == 'object' else 0       # Mengisi kolom kosong dengan string kosong atau angka 0.
    ...
    all_kondisi_cols = [col for col in all_one_hot_cols if col not in df_existing['Alergi Makanan'].fillna('').str.get_dummies(sep=',').columns] # Mengambil kolom kondisi medis.
    all_alergi_cols = [col for col in all_one_hot_cols if col not in df_existing['Kondisi Medis'].fillna('').str.get_dummies(sep=',').columns] # Mengambil kolom alergi makanan.
    ...
    df_kondisi_new = df_kondisi_new.reindex(columns=all_kondisi_cols, fill_value=0)        # Menyesuaikan kolom kondisi medis dengan data training.
    df_alergi_new = df_alergi_new.reindex(columns=all_alergi_cols, fill_value=0)           # Menyesuaikan kolom alergi makanan dengan data training.

    fitur_numerik_new = temp_df_single[['IMT','Usia','Lingkar Perut (cm)',                  # Mengambil fitur numerik dari pasien baru.
                                        'Kadar Lemak Tubuh (%)','Kadar Kolesterol (mg/dL)',
                                        'Aktivitas_enc', 'Berat Badan (kg)', 'Tinggi Badan (m)',
                                        'Denyut Jantung (bpm)', 'Kebutuhan Kalori (kkal)']]

    fitur_numerik_new.fillna(df_existing.mean(numeric_only=True), inplace=True)             # Mengisi nilai kosong dengan rata-rata data training.
...
if selection_mode == 'Pilih Pasien yang Ada':                           # Mengecek apakah pengguna memilih pasien yang sudah ada.
    ...
    if selected_name:                                                   # Mengecek apakah pengguna sudah memilih pasien.
        ...
        st.dataframe(display_df)                                        # Menampilkan data pasien dalam bentuk tabel interaktif.
    ...
    if st.button('Hitung Rekomendasi untuk Data Baru'):                # Mengecek apakah tombol hitung ditekan.
        ...
        new_patient_df = pd.DataFrame([user_data])                     # Mengubah data pasien baru menjadi DataFrame.
        ...
        top_n_similar_indices = [idx for idx, _ in sim_scores_new_patient[:5]] # Mengambil 5 pasien paling mirip.
